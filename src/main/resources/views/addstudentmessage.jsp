            ...
            <div class="row-fluid">
                <div class="span12">
                    ...
                    <form action="addmessage" id="addmessage" method="post">
                        <label>给 <b>${student.name}</b> 同学（编号:<b>${student.id}</b>）微信留言 </label>
                        ...
                        <button class="btn btn-info" id="leavemessage" type="button">
                            <i class="icon-inbox"></i> 留言
                        </button>
                        <button class="btn btn-info" type="button" onclick="location.href='${ctx}/manager/students'">
                    ...
                    <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                        ...
                        <c:forEach items="${studentMessageList}" var="message" varStatus="sta">
                            <tr>
                                <td>${sta.index+1}</td>
                                <td>${message.creatDate}</td>
                                <td>${message.content}</td>
                                <td>
                                    <button class="btn btn-mini btn-danger"
                                            onclick="if(window.confirm('确认删除这条留言？')==true)location.href='${ctx}/manager/deletemessage?studentid=${student.id}&messageid=${message.id}'">
                                        <i class="icon-remove"></i>删除留言
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
