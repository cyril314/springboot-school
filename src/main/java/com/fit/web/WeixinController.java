...
import com.fit.service.WeixinService;
...
public class WeixinController {
    ...
    private static final String TOKEN = "school";
    ...
    public String replyMessage(HttpServletRequest request) {
        ...
        if (checkWeixinReques(request)) {
            ...
            String replyContent = WeixinUtil.WELCOME_CONTENT;
            String type = message.getMsgType();
            if (type.equals(WeixinUtil.TEXT)) {// 仅处理文本回复内容
                String content = message.getContent();// 消息内容
                String[] cs = content.split("_");// 消息内容都以下划线_分隔
                if (cs.length == 2) {
                    try {
                        Long studentid = Long.parseLong(cs[0]);// 学生编号
                        ...
                        replyContent = WeixinUtil.ERROR_CONTENT;
                    }
                }
            }
            // 拼装回复消息
            ...
            reply.setCreatDate(new Date());
            reply.setMsgType(WeixinUtil.TEXT);
